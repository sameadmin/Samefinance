...
*/
//0x8b01dfe686a419ba35638034a0484AF203278BF5
...
contract StrategyWithdraw_SAME is Ownable, ReentrancyGuard, Strategy {
    ...
    constructor(address _lpToken, IESP _esp) public {
        lpToken = _lpToken;
        esp = _esp;
        argID[USDT] = 2;
        argID[USDC] = 1;
    ...
    nonReentrant{
        ...
        // whichWantBack: 0:token0;1:token1
        (,, uint whichWantBack) = abi.decode(data, (address, address, uint));
        ...
        if(debt > 0){
            ...
            esp.remove_liquidity_one_coin(lpToken.balanceOf(address(this)),getArgID(whichToken),0);
            whichToken.safeTransfer(user,whichToken.balanceOf(address(this)));
        }
    }

    function removeError(address whichToken) view public {
        uint256 totalAmounts = lpToken.balanceOf(address(this)).mul(esp.get_virtual_price());
        uint256 removeAmounts = esp.calc_withdraw_one_coin(lpToken.balanceOf(address(this)),getArgID(whichToken)).mul(10 ** 6);
        require(removeAmounts.mul(100) > totalAmounts.mul(99),"esp remove_liquidity_one_coin error");
    }
